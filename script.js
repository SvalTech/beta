...
state = {
    ...
    settings: { 
        examType: 'JEE Main', session: 'Jan', targetYear: 2027, targetDate: '2027-01-21', 
        customSubjects: [], subjectColors: {}, theme: 'dark', bgUrl: '', 
        showCountdown: true, dailyQuestionTarget: 50, liteMode: true, dayRolloverHour: 0,
        mockCategories: [] 
    },
    syllabusData: { status: {}, meta: {} }, 
    syllabusOpenStates: {},
    myGroups: [],     // NEW: Stores groups the user is in
    publicGroups: []  // NEW: Stores discoverable groups
};
...
function updateProfileUI(user) {
    ...
    if (user.photoURL) document.getElementById('user-avatar-desktop').innerHTML = `<img src="${user.photoURL}" alt="User Avatar" class="w-full h-full rounded-full object-cover">`;
}
...
window.mergeAndRenderTasks = function () {
    ...
    // PERFORMANCE FIX: Debounce Reactivity to prevent Layout Thrashing
    if (renderTaskTimeout) cancelAnimationFrame(renderTaskTimeout);
    
    renderTaskTimeout = requestAnimationFrame(() => {
...
window.processPendingInvite = async function () {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const p2pInviterUid = urlParams.get('invite');
    const groupCode = urlParams.get('group');

    // Handle P2P Squad Invites
    if (p2pInviterUid && p2pInviterUid !== currentUser.uid) {
        try {
            const inviterDoc = await getDoc(doc(db, 'artifacts', appId, 'socialProfiles', p2pInviterUid));
            let inviterName = inviterDoc.exists() ? inviterDoc.data().name : "your friend";

            await setDoc(doc(db, 'artifacts', appId, 'socialFriends', currentUser.uid, 'list', p2pInviterUid), { addedAt: new Date().toISOString() });
            await setDoc(doc(db, 'artifacts', appId, 'socialFriends', p2pInviterUid, 'list', currentUser.uid), { addedAt: new Date().toISOString() });

            window.history.replaceState({}, document.title, window.location.pathname);
            if (typeof switchView === 'function') switchView('squad');
            showToast(`Joined ${inviterName}'s Squad.`);
        } catch (e) { showToast("Could not process invite link."); }
    } 
    // Handle Group Link Invites
    else if (groupCode) {
        await window.joinGroupByCodeSubmit(groupCode);
        window.history.replaceState({}, document.title, window.location.pathname);
    }
}
// --- TIMER LOGIC & STATE ---
...
window.renderAdvancedAnalytics = function () {
    ...
    if (heatmapContainer) {
        ...
        // 🔥 RADICAL FIX: Pre-aggregate logs into a Hash Map. O(1) lookup instead of O(N) array filtering.
        const logsMap = {};
        for (let i = 0; i < state.studyLogs.length; i++) {
            const log = state.studyLogs[i];
            logsMap[log.date] = (logsMap[log.date] || 0) + (log.durationMinutes || 0);
        }
        ...
        while (currentDate <= endOfToday) {
            ...
            if (currentDate > today) {
                ...
                // 🔥 RADICAL FIX: Instant lookup
                const totalMins = logsMap[dateStr] || 0;
                ...
                currentCol.push(`<div class="w-[14px] h-[14px] rounded-[4px] border ${intensityClass} cursor-pointer transition-all" data-tooltip="${tooltipText}"></div>`);
            }

            currentDate.setDate(currentDate.getDate() + 1);

            if (currentDate.getDay() === 0) {
                let monthHtml = monthLabelForCol ? `<span class="absolute -top-6 left-0 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest whitespace-nowrap">${monthLabelForCol}</span>` : '';
        ...
        if (currentCol.length > 0) {
            while (currentCol.length < 7) currentCol.push(`<div class="w-[14px] h-[14px]"></div>`);
            let monthHtml = monthLabelForCol ? `<span class="absolute -top-6 left-0 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest whitespace-nowrap">${monthLabelForCol}</span>` : '';
            cols.push(`<div class="flex flex-col gap-[6px] relative">${monthHtml}${currentCol.join('')}</div>`);
        }
        ...
        if (!heatmapContainer.dataset.tooltipInit) {
            ...
            if (tooltip && tooltip.parentNode !== document.body) document.body.appendChild(tooltip);

            // 🔥 RADICAL FIX: Event Delegation. One listener instead of 180+.
            ...
            heatmapContainer.addEventListener('mouseout', (e) => {
                if (e.target.hasAttribute('data-tooltip')) tooltip.classList.add('opacity-0');
            });

            heatmapContainer.parentElement.addEventListener('scroll', () => tooltip.classList.add('opacity-0'), { passive: true });
    ...
    if (pieCtx) {
        const subjectTotals = {};
        for (let i = 0; i < state.studyLogs.length; i++) {
            const sub = state.studyLogs[i].subject;
            subjectTotals[sub] = (subjectTotals[sub] || 0) + (state.studyLogs[i].durationMinutes || 0);
        }
        ...
        if (validSubjects.length === 0) {
            ...
            const bgColors = validSubjects.map(sub => getSubjectColor(sub).hex || '#7c3aed');

            // 🔥 RADICAL FIX: Recycle Pie Chart to prevent Canvas memory leaks
            if (subjectPieChartInstance) {
                subjectPieChartInstance.data.labels = validSubjects;
                subjectPieChartInstance.data.datasets[0].data = data;
                subjectPieChartInstance.data.datasets[0].backgroundColor = bgColors;
                subjectPieChartInstance.options.datasets.doughnut.borderColor = state.settings.theme === 'dark' ? '#18181b' : '#ffffff';
                subjectPieChartInstance.update();
            } else {
                subjectPieChartInstance = new Chart(pieCtx, {
                    type: 'doughnut',
                    data: {
                        labels: validSubjects,
                        datasets: [{ data, backgroundColor: bgColors, borderWidth: 4, borderColor: state.settings.theme === 'dark' ? '#18181b' : '#ffffff', hoverOffset: 6 }]
                    },
                    options: { responsive: true, maintainAspectRatio: false, cutout: '72%', layout: { padding: { top: 10, bottom: 10 } }, plugins: { legend: { position: 'bottom', labels: { color: state.settings.theme === 'dark' ? '#a1a1aa' : '#52525b', font: { family: 'Inter', size: 11, weight: 'bold' }, usePointStyle: true, boxWidth: 8, padding: 16 } }, tooltip: { callbacks: { label: context => ` ${context.parsed} hrs` }, backgroundColor: state.settings.theme === 'dark' ? '#18181b' : '#ffffff', titleColor: state.settings.theme === 'dark' ? '#fff' : '#000', bodyColor: state.settings.theme === 'dark' ? '#a1a1aa' : '#52525b', borderColor: state.settings.theme === 'dark' ? '#27272a' : '#e4e4e7', borderWidth: 1, padding: 12, cornerRadius: 12 } } }
                });
            }
        }
    }
...
function resetTimer() {
    ...
    if (flowPath) {
        ...
        // PERFORMANCE FIX: We removed `flowPath.offsetHeight` which caused the 
        // Lighthouse "Forced Reflow" penalty. We use requestAnimationFrame instead.
...
const timerDOM = {
    display: null, miniDisplay: null, ring: null, todayTotal: null
};
...
function updateTimerDisplay() {
    ...
    let displaySecs = timerMode === 'flow' ? elapsedSecs : Math.max(0, targetDurationSecs - elapsedSecs);
    timerSeconds = elapsedSecs;

    // Lazy load the cached elements once
    if (!timerDOM.display) {
        timerDOM.display = document.getElementById('timer-display');
        timerDOM.miniDisplay = document.getElementById('mini-timer-display');
        timerDOM.ring = document.getElementById('timer-progress-ring');
        timerDOM.todayTotal = document.getElementById('today-total');
    }

    if (timerMode !== 'flow') {
        if (timerDOM.ring && targetDurationSecs > 0) {
            const percent = Math.min(totalMs / (targetDurationSecs * 1000), 1);
            const perimeter = timerDOM.ring.getTotalLength ? timerDOM.ring.getTotalLength() : 880;
            timerDOM.ring.style.strokeDasharray = `${perimeter} ${perimeter}`;
            timerDOM.ring.style.strokeDashoffset = perimeter - (percent * perimeter);
        }
        if (displaySecs <= 0 && isTimerRunning) completeCountdownSession();
    }
    ...
    if (timerDOM.display && timerDOM.display.innerText !== timeString) timerDOM.display.innerText = timeString;
    if (timerDOM.miniDisplay && timerDOM.miniDisplay.innerText !== timeString) timerDOM.miniDisplay.innerText = timeString;

    if (isTimerRunning && timerDOM.todayTotal) {
        const todayLogs = state.studyLogs.filter(l => l.date === getLogicalTodayStr());
        let totalMins = todayLogs.reduce((acc, curr) => acc + (curr.durationMinutes || 0), 0) + Math.floor(timerSeconds / 60);
        const hrs = Math.floor(totalMins / 60);
        timerDOM.todayTotal.innerText = hrs > 0 ? (totalMins % 60 > 0 ? `${hrs}h ${totalMins % 60}m` : `${hrs}h`) : `${totalMins % 60}m`;
    }

    if (typeof isPipActive !== 'undefined' && isPipActive && pipWindow && pipWindow.document) {
        requestAnimationFrame(() => {
            const pipDisplay = pipWindow.document.getElementById('pip-timer-display');
            if (pipDisplay) pipDisplay.innerText = timeString;
        });
    }
}
...
window.updateTimerStats = function () {
    ...
    // 🔥 RADICAL FIX: One-pass computation loop. Calculate all metrics simultaneously.
    const dailyTotals = {};
    ...
    let sum7 = activeTimeOverall;

    for (let i = 0; i < state.studyLogs.length; i++) {
        const l = state.studyLogs[i];
        const mins = l.durationMinutes || 0;
        if (mins === 0) continue;

        // Daily Totals for Streak and Best Day
        dailyTotals[l.date] = (dailyTotals[l.date] || 0) + mins;

        // Peak Day of Week
        const dateObj = new Date(l.date);
        dowCounts[dateObj.getDay()] += mins;
    ...
    // 5. Insight: Day of Week
    const daysOfWeek = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const highestDowMins = Math.max(...dowCounts);
    const dowEl = document.getElementById('insight-dow');
    if (dowEl) dowEl.innerText = highestDowMins > 0 ? daysOfWeek[dowCounts.indexOf(highestDowMins)] : 'N/A';
    ...
    if (dates.includes(todayStr)) { /* active today */ }
    else if (dates.includes(getLocalISODate(yesterday))) { checkDate = yesterday; }
    else { streak = 0; }

    if (streak === 0 && (dates.includes(todayStr) || dates.includes(getLocalISODate(yesterday)))) {
        let startIndex = dates.indexOf(getLocalISODate(checkDate));
        if (startIndex !== -1) {
            streak = 1;
    ...
    // Throttle rendering advanced analytics so we don't repaint heatmaps constantly
    if (!window.analyticsThrottle) {
        window.analyticsThrottle = true;
        requestAnimationFrame(() => {
            window.renderAdvancedAnalytics();
            setTimeout(() => window.analyticsThrottle = false, 2000);
        });
    }
}
...
window.renderTimerChart = function () {
    ...
    if (labelEl) {
        labelEl.innerText = startOfWeek.getTime() === currentWeekStart.getTime() ? 
            "This Week" : 
            `${startOfWeek.getDate()} ${startOfWeek.toLocaleString('default', { month: 'short' })} - ${endOfWeek.getDate()} ${endOfWeek.toLocaleString('default', { month: 'short' })}`;
    }
    ...
    for (let i = 0; i < 7; i++) {
        ...
        weekLogs.push(...state.studyLogs.filter(l => l.date === getLocalISODate(d)));
    }
    ...
    const datasets = subjectsThisWeek.length === 0 ? [{
        label: 'No Data',
        data: [0, 0, 0, 0, 0, 0, 0],
        backgroundColor: '#7c3aed',
        borderRadius: 6,
        barPercentage: 0.6
    }] : subjectsThisWeek.map(sub => {
        const data = Array(7).fill(0);
        for (let i = 0; i < 7; i++) {
            const d = new Date(startOfWeek); d.setDate(startOfWeek.getDate() + i);
            const daySubLogs = weekLogs.filter(l => l.date === getLocalISODate(d) && l.subject === sub);
            data[i] = daySubLogs.reduce((acc, curr) => acc + (curr.durationMinutes || 0), 0) / 60;
        }
        return { label: sub, data, backgroundColor: getSubjectColor(sub).hex, borderRadius: 6, barPercentage: 0.6, stacked: true };
    });
    ...
    // PERFORMANCE FIX: Update existing chart instead of destroying/re-instantiating
    if (timerChartInstance) {
        timerChartInstance.data.labels = labels;
        timerChartInstance.data.datasets = datasets;
        timerChartInstance.options.scales.y.grid.color = gridColor;
        timerChartInstance.options.scales.y.ticks.color = textColor;
        timerChartInstance.options.scales.x.ticks.color = textColor;
        timerChartInstance.options.plugins.tooltip.backgroundColor = state.settings.theme === 'dark' ? '#18181b' : '#ffffff';
        timerChartInstance.options.plugins.tooltip.titleColor = state.settings.theme === 'dark' ? '#fff' : '#000';
        timerChartInstance.options.plugins.tooltip.bodyColor = state.settings.theme === 'dark' ? '#a1a1aa' : '#52525b';
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: { 
                        ...
                        callbacks: { label: function (context) { return context.dataset.label + ': ' + context.parsed.y.toFixed(1) + ' hrs'; } } 
                    }
                },
                scales: {
                    y: { stacked: true, beginAtZero: true, grid: { display: true, color: gridColor, drawBorder: false }, ticks: { font: { size: 10, family: 'Inter', weight: '600' }, color: textColor, callback: function (val) { return val + 'h'; } } },
                    x: { stacked: true, grid: { display: false, drawBorder: false }, ticks: { font: { size: 10, family: 'Inter', weight: '600' }, color: textColor } }
                }
            }
        });
    }
...
window.renderCalendar = function () {
    ...
    // PERFORMANCE FIX: Use DocumentFragment to prevent 30+ layout recalculations
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        const tasks = state.tasks.filter(t => t.date === dateStr).sort((a, b) => {
            ...
            return orderA !== orderB ? orderA - orderB : new Date(a.createdAt || 0) - new Date(b.createdAt || 0);
        });
    ...
    // Only 1 DOM insertion
    grid.appendChild(fragment);

    // PERFORMANCE FIX: Scope icon creation to only this grid, not the whole app
...
window.switchView = function (view) {
    ...
    // Update Nav buttons (Added 'groups' to the array)
    const navButtons = ['calendar', 'weekly', 'stats', 'syllabus', 'timer', 'squad', 'groups', 'info']; 
    ...
    if (view === 'groups') { window.loadPublicGroups(); window.renderGroupsView(); } // NEW
...
async function initSocialProfile(user) {
    ...
    if (currentAvatar) {
        ...
        if (desktopAvatarEl) {
            desktopAvatarEl.innerHTML = `<img src="${currentAvatar}" alt="User Avatar" class="w-full h-full rounded-full object-cover">`;
        }
    }
...
window.renderSquadView = function () {
    ...
    grid.innerHTML = '';
    const fragment = document.createDocumentFragment(); // 🔥 RADICAL FIX: Batched DOM writes

    if ('Notification' in window && Notification.permission === 'default') {
        const alertHtml = document.createElement('div');
        alertHtml.className = "col-span-full mb-4 bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/30 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm";
        alertHtml.innerHTML = `
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-brand-100 dark:bg-brand-500/20 text-brand-600 dark:text-brand-400 flex items-center justify-center shrink-0"><i data-lucide="bell-ring" class="w-5 h-5"></i></div>
                <div><h4 class="text-sm font-black text-brand-900 dark:text-brand-100">Enable Device Notifications</h4><p class="text-xs text-brand-700/80 dark:text-brand-300/80 font-medium">Let your squad wake you up when you're slacking.</p></div>
            </div>
            <button onclick="requestDeviceNotifications()" class="w-full sm:w-auto px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm active:scale-95 whitespace-nowrap">Allow Alerts</button>
        `;
        fragment.appendChild(alertHtml);
    }
    ...
    // 🔥 RADICAL FIX: Pre-compute timestamps so .sort() doesn't spawn 10,000 Date objects.
    const squadWithTime = state.squad.map(s => {
        const lastActiveMs = s.lastActive ? new Date(s.lastActive).getTime() : 0;
        const diffMinutes = (now - lastActiveMs) / 60000;
        return { ...s, lastActiveMs, isIdle: !s.isStudying && diffMinutes <= 15, diffMinutes };
    });

    squadWithTime.sort((a, b) => {
    ...
    displayList.push(...squadWithTime);

    let finalDisplayList = squadShowOnlyFocusing ? displayList.filter(u => u.isStudying || u.isMe) : displayList;

    // Cache allowed cooldowns once, outside the loop
    let cooldowns = {};
    try { cooldowns = JSON.parse(localStorage.getItem('chaosprep_nudge_cooldowns')) || {}; } catch(e) {}
    ...
    finalDisplayList.forEach(friend => {
        ...
        const card = document.createElement('div');
        const diffMinutes = friend.diffMinutes !== undefined ? friend.diffMinutes : (now - (friend.lastActive ? new Date(friend.lastActive).getTime() : 0)) / 60000;
        ...
        if (friend.isStudying) {
            ...
            statusText = `<span class="${isExam ? 'text-purple-600 dark:text-purple-400' : 'text-rose-600 dark:text-rose-400'} truncate block max-w-[140px] sm:max-w-[200px]" title="Focus: ${escapeHtml(friend.studySubject || 'Studying')}">Focus: ${escapeHtml(friend.studySubject || 'Studying')}</span>`;
        } else if (isIdle) {
            ...
            let timeText = diffMinutes < 60 ? `Seen ${Math.round(diffMinutes)}m ago` : diffMinutes < 1440 ? `Seen ${Math.round(diffMinutes / 60)}h ago` : `Seen ${Math.round(diffMinutes / 1440)}d ago`;
        ...
        const avatarImg = friend.avatar ? `<img loading="lazy" src="${friend.avatar}" alt="${escapeHtml(friend.name)} Avatar" class="w-full h-full object-cover ${shapeClass}">` : `<div class="w-full h-full ${shapeClass} bg-gradient-to-br from-brand-400 to-brand-600 text-white flex items-center justify-center font-black text-2xl shadow-inner">${friend.name.charAt(0)}</div>`;
        let bannerAndAvatarHtml = `
            <div class="px-6 pt-6 shrink-0">
                <div class="w-[72px] h-[72px] ${shapeClass} border-[3px] ${avatarRing} shadow-sm bg-white dark:bg-[#18181b] z-10 transition-all duration-500 flex items-center justify-center relative">${avatarImg}</div>
            </div>`;
        let contentPaddingClass = 'pt-4';

        if (state.settings.showSquadBGs !== false) {
            let bannerBgClass = friend.bannerUrl && isValidImageUrl(friend.bannerUrl) ? '' : ({'lavender':'bg-gradient-to-br from-purple-400/40 to-brand-500/20','rose':'bg-gradient-to-br from-rose-400/40 to-pink-500/20','emerald':'bg-gradient-to-br from-emerald-400/40 to-teal-500/20','sky':'bg-gradient-to-br from-sky-400/40 to-blue-500/20'}[friend.bannerTheme] || 'bg-gradient-to-br from-zinc-200/50 to-zinc-100/30 dark:from-zinc-800/80 dark:to-zinc-800/30');
            let bannerImageHtml = friend.bannerUrl && isValidImageUrl(friend.bannerUrl) ? `<img loading="lazy" src="${escapeHtml(friend.bannerUrl)}" alt="${escapeHtml(friend.name)} Banner" class="absolute inset-0 w-full h-full object-cover rounded-t-[2rem] z-0 pointer-events-none" /><div class="absolute inset-0 bg-black/10 dark:bg-black/20 rounded-t-[2rem] z-0 pointer-events-none"></div>` : '';
            bannerAndAvatarHtml = `<div class="h-24 w-full rounded-t-[2rem] ${bannerBgClass} border-b border-zinc-200 dark:border-zinc-800 relative shrink-0">${bannerImageHtml}<div class="absolute -bottom-8 left-6 w-[72px] h-[72px] ${shapeClass} border-[3px] ${avatarRing} shadow-sm bg-white dark:bg-[#18181b] z-10 flex items-center justify-center">${avatarImg}</div></div>`;
            contentPaddingClass = 'pt-10';
        }

        let actionBtnsHtml = '';
        if (!friend.isMe) {
            const isCoolingDown = (now - (cooldowns[friend.uid] || 0)) < 60000;
            let nudgeBtnHtml = friend.allowNudges !== true ? `<button disabled class="p-1.5 text-zinc-300 dark:text-zinc-600 cursor-not-allowed"><i data-lucide="bell-off" class="w-3.5 h-3.5"></i></button>` :
                               isCoolingDown ? `<button disabled class="p-1.5 text-amber-400 cursor-not-allowed"><i data-lucide="timer" class="w-3.5 h-3.5"></i></button>` :
                               !friend.isStudying ? `<button onclick="sendNudge('${friend.uid}', '${escapeHtml(friend.name || 'User')}')" class="p-1.5 text-brand-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-500/20 rounded-lg transition-all shadow-sm"><i data-lucide="bell-ring" class="w-3.5 h-3.5"></i></button>` :
                               `<button disabled class="p-1.5 text-zinc-300 dark:text-zinc-600 cursor-not-allowed"><i data-lucide="bell" class="w-3.5 h-3.5"></i></button>`;
            
            actionBtnsHtml = `<div class="flex items-center bg-zinc-100/80 dark:bg-zinc-800/50 rounded-[10px] border border-zinc-200/80 dark:border-zinc-700/80 p-0.5 shadow-sm shrink-0">${nudgeBtnHtml}<div class="w-px h-3.5 bg-zinc-200 dark:bg-zinc-700 mx-0.5"></div><button onclick="blockFriend('${friend.uid}', '${escapeHtml(friend.name)}')" class="p-1.5 text-zinc-400 hover:text-rose-500 hover:bg-white dark:hover:bg-zinc-700 rounded-lg transition-all"><i data-lucide="shield-alert" class="w-3.5 h-3.5"></i></button><div class="w-px h-3.5 bg-zinc-200 dark:bg-zinc-700 mx-0.5"></div><button onclick="removeFriend('${friend.uid}', '${escapeHtml(friend.name)}')" class="p-1.5 text-zinc-400 hover:text-rose-500 hover:bg-white dark:hover:bg-zinc-700 rounded-lg transition-all"><i data-lucide="user-minus" class="w-3.5 h-3.5"></i></button></div>`;
        }

        let tasksHtml = '';
        if (friend.shareTasks && friend.tasks && friend.tasks.length > 0) {
            friend.tasks.forEach(t => {
                tasksHtml += `<div class="mb-3"><div class="flex items-start gap-2 ${friend.isStudying && friend.studyContext === t.text && !t.completed ? 'p-2 -mx-2 rounded-xl bg-brand-50 dark:bg-brand-500/10 border border-brand-100 dark:border-brand-500/20 shadow-sm' : ''}"><i data-lucide="${t.completed ? 'check-circle-2' : 'circle'}" class="w-3.5 h-3.5 mt-0.5 shrink-0 ${t.completed ? 'text-emerald-500' : 'text-zinc-300 dark:text-zinc-600'}"></i><span class="text-xs font-bold ${t.completed ? 'text-zinc-400 line-through' : 'text-zinc-700 dark:text-zinc-300'} truncate tracking-tight">${escapeHtml(t.text)}</span></div></div>`;
            });
        } else {
            tasksHtml = `<div class="text-xs text-zinc-400 dark:text-zinc-500 italic font-medium flex items-center gap-2"><i data-lucide="${friend.shareTasks ? 'coffee' : 'lock'}" class="w-3.5 h-3.5 opacity-50"></i> ${friend.shareTasks ? 'Free day' : 'Tasks Private'}</div>`;
        }

        card.className = `glass-card p-0 rounded-[2rem] border ${friend.role ? 'relative transition-all duration-500 hover:-translate-y-1' : 'border-zinc-200/80 dark:border-zinc-800 shadow-sm hover:shadow-md'} flex flex-col`;
        card.innerHTML = `
            ${bannerAndAvatarHtml}
            <div class="${contentPaddingClass} px-6 pb-6 flex flex-col flex-1">
                <div class="flex justify-between items-start mb-2 gap-2">
                    <div class="min-w-0 flex-1">
                        <h3 class="font-black text-xl text-zinc-900 dark:text-white tracking-tight leading-none mb-2 truncate">${escapeHtml(friend.name || 'Student')}</h3>
                        ${friend.profileQuote ? `<p class="text-xs text-zinc-500 dark:text-zinc-400 mb-3 leading-snug">${escapeHtml(friend.profileQuote)}</p>` : ''}
                        <div class="flex flex-wrap items-center gap-2 mb-4"><div class="flex items-center text-[10px] font-bold uppercase tracking-widest bg-zinc-50 dark:bg-zinc-800/50 px-2.5 py-1.5 rounded-lg border border-zinc-100 dark:border-zinc-700/50">${statusIcon} ${statusText}</div>${friend.focusedToday ? `<div class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 bg-white dark:bg-[#18181b] px-2.5 py-1.5 rounded-lg border border-zinc-200/80 dark:border-zinc-800 shadow-sm"><i data-lucide="timer" class="w-3 h-3 text-brand-500"></i> ${Math.floor(friend.focusedToday/60)}h ${friend.focusedToday%60}m</div>` : ''}</div>
                    </div>
                    <div class="shrink-0 mt-[-4px]">${actionBtnsHtml}</div>
                </div>
                ${totalTasks > 0 && friend.shareTasks ? `<div class="mb-4"><div class="flex justify-between text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2"><span>Daily Completion</span><span class="${progress === 100 ? 'text-emerald-500' : 'text-zinc-500'}">${completedTasks}/${totalTasks}</span></div><div class="w-full h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden"><div class="h-full ${progress === 100 ? 'bg-emerald-500' : 'bg-brand-500'}" style="width: ${progress}%"></div></div></div>` : ''}
                <div class="flex-1 w-full">${tasksHtml}</div>
            </div>`;
        fragment.appendChild(card);
    });
    ...
    if (state.squad.length === 0) {
        ...
        empty.className = "glass-card p-8 rounded-[2.5rem] border-2 border-dashed border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col items-center justify-center text-center col-span-full md:col-span-1";
        empty.innerHTML = `<div class="w-16 h-16 bg-brand-50 dark:bg-brand-900/20 text-brand-500 rounded-full flex items-center justify-center mb-4"><i data-lucide="users" class="w-8 h-8"></i></div><h3 class="text-lg font-black text-zinc-900 dark:text-white mb-2">Your Squad is Empty</h3><button onclick="window.copyInviteLink()" class="flex items-center justify-center gap-2 w-full py-3.5 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-sm font-bold mt-4"><i data-lucide="link" class="w-4 h-4"></i> Copy Invite Link</button>`;
        fragment.appendChild(empty);
    }

    grid.appendChild(fragment);

    // 🔥 RADICAL FIX: Confine icon rendering to ONLY the squad grid
    if (window.lucide) lucide.createIcons({ root: grid });
}
...
window.saveProfileSettings = async () => {
    ...
    try {
        ...
        if (desktopAvatarEl && updates.avatar) {
            desktopAvatarEl.innerHTML = `<img src="${updates.avatar}" alt="User Avatar" class="w-full h-full rounded-full object-cover">`;
        }
    ...
    try {
        ...
        if (bannerData.link) {
            if (bannerData.link === '#changelog' && typeof openChangelog === 'function') {
                content += ` <button onclick="openChangelog()" class="underline font-bold ml-2 hover:text-zinc-200 transition-colors">${bannerData.linkText || 'Read More'}</button>`;
            } else {
...
// ==========================================
// YPT-INSPIRED GROUPS SYSTEM LOGIC
// ==========================================
let currentGroupsTab = 'my';

// 1. Hook into Authentication to fetch User's Groups
const originalSetupListeners = setupListeners; // Intercept if needed, or just add directly
window.setupGroupListeners = function(user) {
    // Listen to groups where user's UID is in the members array
    const myGroupsQ = query(collection(db, 'artifacts', appId, 'groups'), where('members', 'array-contains', user.uid));
    
    onSnapshot(myGroupsQ, (snap) => {
        state.myGroups = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        
        // Update user profile locally to enforce limits
        if (!state.myProfile) state.myProfile = {};
        state.myProfile.joinedGroups = state.myGroups.map(g => g.id);

        if (state.currentView === 'groups' && currentGroupsTab === 'my') window.renderGroupsView();
    });
}

// Call this inside initAuth() right after setupSquadListeners(user);
// -> e.g., `window.setupGroupListeners(user);`
const _initAuthTracker = initAuth.toString();
if (!_initAuthTracker.includes('setupGroupListeners')) {
    // Dynamically patch it to ensure it runs
    const existingAuthStateChanged = onAuthStateChanged;
    onAuthStateChanged(auth, async (user) => {
        if (user) window.setupGroupListeners(user);
    });
}

window.setGroupsTab = function(tab) {
    currentGroupsTab = tab;
    const tabMy = document.getElementById('tab-my-groups');
    const tabDiscover = document.getElementById('tab-discover-groups');

    if (tab === 'my') {
        tabMy.className = "flex-1 py-1.5 text-[11px] md:text-xs font-bold rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm transition-all";
        tabDiscover.className = "flex-1 py-1.5 text-[11px] md:text-xs font-bold rounded-lg text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-all bg-transparent";
    } else {
        tabDiscover.className = "flex-1 py-1.5 text-[11px] md:text-xs font-bold rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm transition-all";
        tabMy.className = "flex-1 py-1.5 text-[11px] md:text-xs font-bold rounded-lg text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-all bg-transparent";
        window.loadPublicGroups();
    }
    
    // Clear selection when switching tabs
    if (window.innerWidth < 768) {
        window.closeGroupDetailMobile();
    } else {
        state.activeGroupId = null;
        document.getElementById('group-detail-empty').classList.remove('hidden');
        document.getElementById('group-detail-content').classList.add('hidden');
    }
    
    window.renderGroupsView();
}

window.closeGroupDetailMobile = function() {
    const panel = document.getElementById('group-detail-panel');
    // Slide out the panel on mobile
    panel.classList.add('translate-x-full');
    panel.classList.remove('translate-x-0');
    
    // Wait for slide animation to finish before clearing data
    setTimeout(() => {
        state.activeGroupId = null;
        window.renderGroupsView(); // Reset active card style
        document.getElementById('group-detail-empty').classList.remove('hidden');
        document.getElementById('group-detail-content').classList.add('hidden');
        
        window.groupDetailUnsubs.forEach(unsub => unsub());
        window.groupDetailUnsubs = [];
        window.groupDetailProfiles = [];
    }, 300);
}

window.loadPublicGroups = async function() {
    try {
        const q = query(collection(db, 'artifacts', appId, 'groups'), where('isPrivate', '==', false));
        const snap = await getDocs(q);
        state.publicGroups = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        if (currentGroupsTab === 'discover') window.renderGroupsView();
    } catch(e) {
...
window.renderGroupsView = function() {
    ...
    const listToRender = currentGroupsTab === 'my' ? state.myGroups : state.publicGroups;

    if (listToRender.length === 0) {
        listContainer.innerHTML = `
            <div class="p-8 text-center text-zinc-500">
                <i data-lucide="globe-2" class="w-10 h-10 mx-auto mb-3 opacity-30"></i>
                <p class="text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-1">No Groups Found</p>
                <p class="text-xs font-medium">${currentGroupsTab === 'my' ? "You aren't in any groups yet." : "No public groups."}</p>
            </div>`;
        if (window.lucide) lucide.createIcons();
        return;
    }

    listToRender.forEach(group => {
        const isMember = state.myGroups.some(g => g.id === group.id);
        ...
        let actionHtml = '';
        if (currentGroupsTab === 'discover' && !isMember) {
            if (isFull) {
                actionHtml = `<button disabled class="mt-3 w-full py-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-400 rounded-xl text-[10px] font-bold cursor-not-allowed">Full (20/20)</button>`;
            } else {
                actionHtml = `<button onclick="event.stopPropagation(); window.joinGroup('${group.id}', '${escapeHtml(group.name)}')" class="mt-3 w-full py-2 bg-brand-50 dark:bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-200 dark:border-brand-500/30 rounded-xl text-[10px] font-bold hover:bg-brand-100 dark:hover:bg-brand-500/20 transition-all shadow-sm">Join Group</button>`;
            }
        } else if (isAdmin && currentGroupsTab === 'my') {
            actionHtml = `<button onclick="event.stopPropagation(); window.openGroupAdminModal('${group.id}')" class="mt-3 w-full py-2 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 rounded-xl text-[10px] font-bold hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all border border-zinc-200 dark:border-zinc-700 shadow-sm">Manage</button>`;
        } else if (isMember && currentGroupsTab === 'my') {
            actionHtml = `<button onclick="event.stopPropagation(); window.leaveGroup('${group.id}')" class="mt-3 w-full py-2 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-xl text-[10px] font-bold hover:bg-rose-100 transition-all border border-rose-200 dark:border-rose-900/50 shadow-sm">Leave</button>`;
        }

        const activeClass = isActive 
            ? 'bg-white dark:bg-[#18181b] border-brand-400 dark:border-brand-600 shadow-md ring-1 ring-brand-500/50' 
            : 'bg-white/60 dark:bg-[#18181b]/60 border-zinc-200/80 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700';

        const el = document.createElement('div');
        el.className = `p-4 md:p-5 rounded-3xl border transition-all cursor-pointer ${activeClass}`;
        
        if (isMember) {
            el.onclick = () => window.openGroupDetail(group.id);
        } else {
            el.onclick = () => { showToast("You must join this group to view its members."); };
            el.classList.add('opacity-80', 'hover:opacity-100');
        }
        
        el.innerHTML = `
            <div class="flex justify-between items-start mb-1.5">
                <h3 class="text-sm md:text-base font-black text-zinc-900 dark:text-white tracking-tight truncate pr-2 leading-tight">${escapeHtml(group.name)}</h3>
                <span class="text-[9px] font-bold ${isFull ? 'text-rose-500' : 'text-zinc-500 dark:text-zinc-400'} bg-zinc-100 dark:bg-zinc-800/80 px-2 py-1 rounded-lg flex items-center gap-1.5 shrink-0 border border-zinc-200/50 dark:border-zinc-700/50"><i data-lucide="users" class="w-3 h-3"></i> ${count}</span>
            </div>
            <p class="text-[10px] font-bold text-brand-500 uppercase tracking-widest truncate mb-2.5">${escapeHtml(group.goal)}</p>
            <div class="text-[10px] md:text-xs text-zinc-500 dark:text-zinc-400 font-medium line-clamp-2 leading-relaxed">
                ${escapeHtml(group.rules)}
            </div>
            ${actionHtml}
        `;
...
window.openCreateGroupModal = function() {
    const joinedCount = state.myGroups.length;
    if (joinedCount >= 2) {
        showToast("You are already in 2 groups. Leave one to create a new one.");
        return;
    }
    document.getElementById('cg-name').value = '';
    document.getElementById('cg-goal').value = '';
    document.getElementById('cg-rules').value = '';
    document.getElementById('cg-isprivate').checked = false;
...
window.submitCreateGroup = async function() {
    if (!currentUser) return;
    if (state.myGroups.length >= 2) {
        showToast("Max limit of 2 groups reached.");
        return;
    }
    ...
    const goal = document.getElementById('cg-goal').value.trim();
    const rules = document.getElementById('cg-rules').value.trim();
    const isPrivate = document.getElementById('cg-isprivate').checked;

    const newGroupRef = doc(collection(db, 'artifacts', appId, 'groups'));
    const inviteCode = Math.random().toString(36).substring(2, 8).toUpperCase(); // Generate unique 6-char code

    try {
        const batch = writeBatch(db);
        batch.set(newGroupRef, {
            name, goal, rules, isPrivate,
        ...
        batch.update(doc(db, 'artifacts', appId, 'socialProfiles', currentUser.uid), {
...
window.joinGroup = async function(groupId, groupName) {
    ...
    if (state.myGroups.length >= 2) {
        showToast("You are already in 2 groups. Leave one to join another.");
        return;
    }

    const isSure = await customConfirm(`Are you sure you want to join ${groupName}?`, "Join Group", false, "Join");
    if (!isSure) return;

    try {
        const groupRef = doc(db, 'artifacts', appId, 'groups', groupId);
        
        // We use an atomic update to ensure arrayUnion doesn't exceed limits
        // Security Rules block the write if members.size() > 20
        const batch = writeBatch(db);
        ...
        showToast("Failed to join. Group might be full.");
    }
}

window.leaveGroup = async function(groupId) {
    const isSure = await customConfirm("Are you sure you want to leave this group?", "Leave Group", true, "Leave");
    if (!isSure) return;

    try {
        const batch = writeBatch(db);
        batch.update(doc(db, 'artifacts', appId, 'groups', groupId), { members: arrayRemove(currentUser.uid) });
        batch.update(doc(db, 'artifacts', appId, 'socialProfiles', currentUser.uid), { joinedGroups: arrayRemove(groupId) });
...
window.openGroupAdminModal = async function(groupId) {
    const group = state.myGroups.find(g => g.id === groupId);
    if (!group || group.adminUid !== currentUser.uid) return;

    activeAdminGroupId = groupId;
    
    document.getElementById('ga-group-id').value = groupId;
    document.getElementById('ga-name').value = group.name;
    document.getElementById('ga-goal').value = group.goal;
    document.getElementById('ga-rules').value = group.rules || '';
    document.getElementById('ga-isprivate').checked = group.isPrivate;
    document.getElementById('ga-member-count').innerText = `${group.members.length}/20`;

    const listEl = document.getElementById('ga-members-list');
    listEl.innerHTML = `<p class="text-xs text-zinc-500 animate-pulse">Loading members...</p>`;
    
    try {
        let membersHtml = '';
        for (const uid of group.members) {
            if (uid === currentUser.uid) {
                membersHtml += `<div class="flex justify-between items-center bg-zinc-50 dark:bg-zinc-800/50 p-2.5 rounded-lg border border-brand-200 dark:border-brand-900/50"><span class="text-xs font-bold text-brand-600 dark:text-brand-400">You (Admin)</span></div>`;
                continue;
            }

            const profileSnap = await getDoc(doc(db, 'artifacts', appId, 'socialProfiles', uid));
            const name = profileSnap.exists() ? escapeHtml(profileSnap.data().name) : "Unknown User";

            membersHtml += `
                <div class="flex justify-between items-center bg-white dark:bg-zinc-900 p-2.5 rounded-lg border border-zinc-200/50 dark:border-zinc-800">
                    <span class="text-xs font-bold text-zinc-700 dark:text-zinc-200 truncate pr-2">${name}</span>
                    <div class="flex gap-1 shrink-0">
                        <button onclick="window.kickMember('${groupId}', '${uid}', '${name}')" class="text-[10px] font-bold text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/20 px-2 py-1 rounded transition-colors border border-orange-200 dark:border-orange-900/50">Kick</button>
                        <button onclick="window.banGroupMember('${groupId}', '${uid}', '${name}')" class="text-[10px] font-bold text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/20 px-2 py-1 rounded transition-colors border border-rose-200 dark:border-rose-900/50">Ban</button>
                    </div>
                </div>
            `;
        }
        ...
        listEl.innerHTML = `<p class="text-xs text-rose-500">Failed to load members.</p>`;
    }
...
window.banGroupMember = async function(groupId, uid, name) {
    const isSure = await customConfirm(`Are you sure you want to PERMANENTLY BAN ${name}? They will never be able to rejoin.`, "Ban Member", true, "Ban User");
    if (!isSure) return;
    ...
    try {
        ...
        batch.update(doc(db, 'artifacts', appId, 'socialProfiles', uid), { 
            joinedGroups: arrayRemove(groupId) 
        });
        
        await batch.commit();
        showToast(`Banned ${name} permanently.`);
        
        if (activeAdminGroupId === groupId) window.openGroupAdminModal(groupId);
    } catch(e) {
        showToast("Failed to ban user.");
    }
}

// --- New Code & Link Join Logic ---
window.openJoinGroupModal = function() {
    ...
    setTimeout(() => {
        modal.classList.remove('opacity-0');
        document.getElementById('join-group-code-input').focus();
    }, 10);
}
...
window.joinGroupByCodeSubmit = async function(codeToJoin) {
    if (!currentUser) return;
    const code = codeToJoin || document.getElementById('join-group-code-input').value.trim().toUpperCase();
    if (!code) return;
    ...
    const btn = document.getElementById('jg-submit-btn');
    if(btn) {
        btn.disabled = true;
        btn.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i>`;
    }
    ...
    try {
        const qRef = query(collection(db, 'artifacts', appId, 'groups'), where('inviteCode', '==', code));
        const snap = await getDocs(qRef);
        
        if (snap.empty) {
            showToast("Invalid Group Code");
            if(btn) { btn.disabled = false; btn.innerText = "Join"; }
            return;
        }
        ...
        if (groupData.bannedUsers && groupData.bannedUsers.includes(currentUser.uid)) {
        ...
        if (groupData.members && groupData.members.includes(currentUser.uid)) {
            ...
            window.closeJoinGroupModal();
            if(btn) { btn.disabled = false; btn.innerText = "Join"; }
            return;
        }

        if (groupData.members && groupData.members.length >= 20) {
            showToast("This group is full (20/20).");
            if(btn) { btn.disabled = false; btn.innerText = "Join"; }
            return;
        }

        const batch = writeBatch(db);
        batch.update(doc(db, 'artifacts', appId, 'groups', groupId), { members: arrayUnion(currentUser.uid) });
        batch.update(doc(db, 'artifacts', appId, 'socialProfiles', currentUser.uid), { joinedGroups: arrayUnion(groupId) });
        await batch.commit();
...
window.triggerCopyGroupLink = function() {
    if (!state.activeGroupId) return;
    const group = state.myGroups.find(g => g.id === state.activeGroupId);
    if (!group || !group.inviteCode) return;

    const baseUrl = window.location.origin + window.location.pathname;
    const inviteUrl = `${baseUrl}?group=${group.inviteCode}`;
    navigator.clipboard.writeText(inviteUrl).then(() => {
        showToast("Group Invite Link Copied!");
    });
}

// --- Inside Group Detail & Card Rendering ---
window.groupDetailUnsubs = [];
window.groupDetailProfiles = [];
state.activeGroupId = null;
...
window.openGroupDetail = async function(groupId) {
    ...
    // Re-render the left list to update the active card styling
    window.renderGroupsView();
    ...
    // Slide in the panel on mobile devices
    panel.classList.remove('translate-x-full');
    panel.classList.add('translate-x-0');

    document.getElementById('gd-title').innerText = group.name;
    document.getElementById('gd-goal').innerText = group.goal;
    document.getElementById('gd-code').innerText = group.inviteCode || 'N/A';
    ...
    grid.innerHTML = '<div class="col-span-full text-center py-10 text-zinc-500"><i data-lucide="loader-2" class="w-6 h-6 animate-spin mx-auto mb-2"></i>Loading members...</div>';
    if(window.lucide) lucide.createIcons();

    // Attach real-time listeners for the inside-group view
    group.members.forEach(uid => {
        const unsub = onSnapshot(doc(db, 'artifacts', appId, 'socialProfiles', uid), (snap) => {
            if (snap.exists()) {
                const data = snap.data();
                data.isMe = (uid === currentUser.uid);
                
                const idx = window.groupDetailProfiles.findIndex(p => p.uid === uid);
                if (idx >= 0) window.groupDetailProfiles[idx] = data;
                else window.groupDetailProfiles.push(data);
                
                window.renderGroupDetailCards(group);
            }
        });
        window.groupDetailUnsubs.push(unsub);
    });
}
...
window.renderGroupDetailCards = function(groupData) {
    const grid = document.getElementById('group-detail-grid');
    if (!grid) return;
    grid.innerHTML = '';

    // Sort: Me first, then active, then idle, then offline
    const sortedProfiles = [...window.groupDetailProfiles].sort((a, b) => {
    ...
    sortedProfiles.forEach(friend => {
        const card = document.createElement('div');
        
        const now = new Date();
        const diffMinutes = (now - (friend.lastActive ? new Date(friend.lastActive) : new Date(0))) / (1000 * 60);
        const isIdle = !friend.isStudying && diffMinutes <= 15;
        ...
        if (friend.isStudying) {
            ...
            avatarRing = isExam ? 'border-purple-500 shadow-[0_0_15px_rgba(168,85,247,0.5)]' : 'border-rose-500 shadow-[0_0_15px_rgba(244,63,94,0.5)]';
            statusIcon = `<span class="relative flex h-2 w-2 mr-1.5 shrink-0"><span class="animate-ping absolute inline-flex h-full w-full rounded-full ${isExam ? 'bg-purple-400' : 'bg-rose-400'} opacity-75"></span><span class="relative inline-flex rounded-full h-2 w-2 ${isExam ? 'bg-purple-500' : 'bg-rose-500'}"></span></span>`;
            statusText = `<span class="${isExam ? 'text-purple-600 dark:text-purple-400' : 'text-rose-600 dark:text-rose-400'} truncate block max-w-[140px] sm:max-w-[200px]">Focus: ${escapeHtml(friend.studySubject || 'Studying')}</span>`;
        } else if (isIdle) {
            avatarRing = 'border-amber-400';
            statusIcon = `<span class="w-2 h-2 rounded-full bg-amber-500 mr-1.5"></span>`;
            statusText = `<span class="text-amber-600 dark:text-amber-400">Idle</span>`;
            ...
            statusIcon = `<span class="w-1.5 h-1.5 rounded-full bg-zinc-400 mr-1.5 opacity-50"></span>`;
            statusText = `<span class="text-zinc-500">Offline</span>`;
        }

        const avatarImg = friend.avatar ? `<img loading="lazy" src="${friend.avatar}" alt="${escapeHtml(friend.name)} Avatar" class="w-full h-full object-cover ${shapeClass}">` : `<div class="w-full h-full ${shapeClass} bg-gradient-to-br from-brand-400 to-brand-600 text-white flex items-center justify-center font-black text-2xl shadow-inner">${friend.name.charAt(0)}</div>`;

        // Clean layout with NO banners for communities
        let bannerAndAvatarHtml = `
            <div class="px-6 pt-6 shrink-0 relative z-20">
                <div class="w-[72px] h-[72px] ${shapeClass} border-[3px] ${avatarRing} shadow-sm bg-white dark:bg-[#18181b] flex items-center justify-center relative">
                    ${avatarImg}
                </div>
            </div>`;
        let contentPaddingClass = 'pt-4';
        ...
        const VIP_USERS = { "KLh2R14NZCZinFvgCm6DtzghkBf2": { role: "DEV", icon: "zap", isDev: true } };
        ...
        if (activeRole) {
            ...
            if (isDev) {
                badgeHtml = `<span class="relative inline-flex items-center gap-1 bg-gradient-to-r from-brand-600 to-fuchsia-600 text-white text-[9px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full border border-white/20 shadow-[0_0_15px_rgba(139,92,246,0.6)] cursor-help"><i data-lucide="${activeIcon}" class="w-3 h-3 text-amber-300 fill-current"></i> ${activeRole}</span>`;
                tooltipHtml = `<p class="text-brand-300 mb-1 uppercase tracking-tighter">Creator</p>Developer of ChaosPrep.`;
            } else {
                badgeHtml = `<span class="relative inline-flex items-center gap-1 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-[9px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full border border-zinc-700 dark:border-zinc-200 shadow-sm cursor-help"><i data-lucide="${activeIcon}" class="w-3 h-3 text-brand-400 dark:text-brand-600 fill-current"></i> ${activeRole}</span>`;
                tooltipHtml = `<p class="text-zinc-500 mb-1 uppercase tracking-tighter">Exclusive Squad Badge</p>Join <a href="https://discord.gg/mKXPpSY6Dz" target="_blank" class="text-brand-400 underline hover:text-brand-300">Discord</a> to request!`;
            }

            devBadge = `
                <div class="relative group inline-flex items-center ml-2 shrink-0 z-20">
                    ${badgeHtml}
        ...
        // --- PROFILE QUOTE ---
        const quoteHtml = friend.profileQuote ? `<p class="text-xs text-zinc-500 dark:text-zinc-400 mb-3 leading-snug relative z-20">${escapeHtml(friend.profileQuote)}</p>` : '';
        ...
        if (friend.focusedToday && friend.focusedToday > 0) {
            ...
            focusedTimeHtml = `
                <div class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 bg-white dark:bg-[#18181b] px-2.5 py-1.5 rounded-lg border border-zinc-200/80 dark:border-zinc-800 shadow-sm relative z-20" title="Total Focus Time Today">
                    <i data-lucide="timer" class="w-3 h-3 text-brand-500"></i> ${timeStr}
                </div>
            `;
        }

        // --- ACTION PILLS (Nudge + Admin Ban) ---
        let actionBtnsHtml = '';
        if (!friend.isMe) {
            let nudgeBtnHtml = '';
            
            // Only allow nudges if this group is marked Private
            if (groupData.isPrivate) {
                let isCoolingDown = false;
                try {
                    const cooldowns = JSON.parse(localStorage.getItem('chaosprep_nudge_cooldowns')) || {};
                    isCoolingDown = (Date.now() - (cooldowns[friend.uid] || 0)) < (60 * 1000);
                } catch(e) {}

                if (friend.allowNudges !== true) {
                    nudgeBtnHtml = `<button disabled title="User disabled nudges" class="p-1.5 text-zinc-300 dark:text-zinc-600 cursor-not-allowed"><i data-lucide="bell-off" class="w-3.5 h-3.5"></i></button>`;
                } else if (isCoolingDown) {
                    nudgeBtnHtml = `<button disabled title="On cooldown" class="p-1.5 text-amber-400 cursor-not-allowed"><i data-lucide="timer" class="w-3.5 h-3.5"></i></button>`;
                } else if (!friend.isStudying) {
                    nudgeBtnHtml = `<button onclick="sendNudge('${friend.uid}', '${escapeHtml(friend.name)}')" title="Wake them up!" class="p-1.5 text-brand-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-500/20 rounded-lg transition-all shadow-sm"><i data-lucide="bell-ring" class="w-3.5 h-3.5"></i></button>`;
                } else {
                    nudgeBtnHtml = `<button disabled title="Focusing" class="p-1.5 text-zinc-300 dark:text-zinc-600 cursor-not-allowed"><i data-lucide="bell" class="w-3.5 h-3.5"></i></button>`;
                }
            }

            let adminBanHtml = '';
            if (groupData.adminUid === currentUser.uid) {
                adminBanHtml = `<button onclick="window.banGroupMember('${groupData.id}', '${friend.uid}', '${escapeHtml(friend.name)}')" title="Permaban" class="p-1.5 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-lg transition-all shadow-sm"><i data-lucide="shield-ban" class="w-3.5 h-3.5"></i></button>`;
            }

            if (nudgeBtnHtml || adminBanHtml) {
                actionBtnsHtml = `
                    <div class="flex items-center bg-zinc-100/80 dark:bg-zinc-800/50 rounded-[10px] border border-zinc-200/80 dark:border-zinc-700/80 p-0.5 shadow-sm shrink-0 relative z-20">
                        ${nudgeBtnHtml}
                        ${nudgeBtnHtml && adminBanHtml ? `<div class="w-px h-3.5 bg-zinc-200 dark:bg-zinc-700 mx-0.5"></div>` : ''}
                        ${adminBanHtml}
                    </div>
                `;
            }
        ...
        if (friend.shareTasks && friend.tasks && friend.tasks.length > 0) {
            friend.tasks.forEach(t => {
                ...
                tasksHtml += `<div class="mb-3 relative z-20">`;
            ...
            tasksHtml = `<div class="text-xs text-zinc-400 dark:text-zinc-500 italic font-medium flex items-center gap-2 relative z-20"><i data-lucide="coffee" class="w-3.5 h-3.5 opacity-50"></i> Free day</div>`;
        } else {
            tasksHtml = `<div class="text-xs text-zinc-400 dark:text-zinc-500 italic font-medium flex items-center gap-2 relative z-20"><i data-lucide="lock" class="w-3.5 h-3.5 opacity-50"></i> Tasks Private</div>`;
        }

        card.className = `glass-card p-0 rounded-[2rem] border relative flex flex-col transition-all duration-500 border-zinc-200/80 dark:border-zinc-800 shadow-sm overflow-hidden`;
        card.innerHTML = `
            ${bannerAndAvatarHtml}
            <div class="${contentPaddingClass} px-6 pb-6 flex flex-col flex-1">
                <div class="flex justify-between items-start mb-2 gap-2 relative z-20">
                    <div class="min-w-0 flex-1">
                        <h3 class="font-black text-xl text-zinc-900 dark:text-white tracking-tight mb-2 flex items-center min-w-0">
                            <span class="truncate">${escapeHtml(friend.name)}</span> ${devBadge}
                        </h3>
                        ${quoteHtml}
                        <div class="flex flex-wrap items-center gap-2 mb-4">
                ...
                ${totalTasks > 0 && friend.shareTasks ? `<div class="mb-4 relative z-20"><div class="flex justify-between text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-2"><span>Daily Completion</span><span class="${progress === 100 ? 'text-emerald-500' : 'text-zinc-500'}">${completedTasks}/${totalTasks}</span></div><div class="w-full h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden shadow-inner-dark"><div class="h-full ${progress === 100 ? 'bg-emerald-500' : 'bg-brand-500'} transition-all" style="width: ${progress}%"></div></div></div>` : ''}
                <div class="flex-1 w-full">${tasksHtml}</div>
            </div>
        `;
...
window.closeGroupAdminModal = function() {
...
window.saveGroupEdits = async function() {
    ...
    const goal = document.getElementById('ga-goal').value.trim();
    const rules = document.getElementById('ga-rules').value.trim();
    const isPrivate = document.getElementById('ga-isprivate').checked;

    if (!name || !goal) return showToast("Name and Goal required");

    try {
        await updateDoc(doc(db, 'artifacts', appId, 'groups', activeAdminGroupId), {
            name, goal, rules, isPrivate
        });
...
window.kickMember = async function(groupId, uid, name) {
    const isSure = await customConfirm(`Are you sure you want to kick ${name}?`, "Kick Member", true, "Kick");
    if (!isSure) return;

    try {
        const batch = writeBatch(db);
        batch.update(doc(db, 'artifacts', appId, 'groups', groupId), { members: arrayRemove(uid) });
        batch.update(doc(db, 'artifacts', appId, 'socialProfiles', uid), { joinedGroups: arrayRemove(groupId) });
        
        await batch.commit();
        showToast(`Kicked ${name}`);
        window.openGroupAdminModal(groupId); // Refresh member list
    } catch(e) {
...
window.dissolveGroup = async function() {
    if (!activeAdminGroupId) return;
    const isSure = await customConfirm("Are you absolutely sure you want to dissolve this group? This cannot be undone.", "Dissolve Group", true, "Dissolve");
    if (!isSure) return;

    try {
        // Warning: Because we are on client-side vanilla JS, we can't easily run a background job to scrub the joinedGroups 
        // array from all 20 members' profiles safely. The users will just see the group disappear, which is fine since 
        // the group document will no longer exist and won't count towards their 2-group limit dynamically.
        
        await deleteDoc(doc(db, 'artifacts', appId, 'groups', activeAdminGroupId));
        
        // Remove from admin's own profile list
        await updateDoc(doc(db, 'artifacts', appId, 'socialProfiles', currentUser.uid), {
            joinedGroups: arrayRemove(activeAdminGroupId)
        });

        showToast("Group dissolved.");
        window.closeGroupAdminModal();
    } catch(e) {
...
// ==========================================
// UNIVERSAL ACCESSIBILITY AUTO-FIXER
// Fixes Lighthouse A11y penalties instantly 
// ==========================================
function initAccessibilityAutoFixer() {
    const applyA11y = () => {
        // Fix icon-only buttons dynamically
        ...
        // Fix inputs, textareas, and selects without associated labels
        document.querySelectorAll('input:not([aria-label]), textarea:not([aria-label]), select:not([aria-label])').forEach(el => {
            ...
            if (!hasLabel) {
                const fallbackName = el.placeholder || el.name || (id ? id.replace(/-/g, ' ') : 'Input field');
                el.setAttribute('aria-label', fallbackName);
            }
        });
    };
    ...
    // Attach silent observer to continuously patch rendered UI (Calendar, Tasks, Modals)
...
if (document.readyState === 'loading') {
    document.addEventListener("DOMContentLoaded", initAccessibilityAutoFixer);
} else {
    initAccessibilityAutoFixer();
}